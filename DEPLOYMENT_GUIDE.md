# Digital Solari ‚Ä?Production Deployment Guide

You're ready to ship! Here's the step-by-step checklist to get **Digital Solari** live at `your-domain.com`.

---

## ‚ú?Phase 1: Code Hardening (DONE)

Code is production-ready:
- ‚ú?WebSocket scheme detects `https:` and uses `wss://` automatically
- ‚ú?QR code URL uses `https` in production, `http` in dev
- ‚ú?dotenv support for environment variables
- ‚ú?Sentry error monitoring (server + client)
- ‚ú?PostHog analytics (6 key events tracked)
- ‚ú?Supabase session persistence (survives restarts)

**Status:** All code changes committed to `main`.

---

## üìã Phase 2: Manual Setup (Do These Next)

### Step 1: Create Supabase Account & Project
1. Go to [supabase.com](https://supabase.com)
2. Sign up / Sign in
3. Create a new project (free tier is fine for start-up phase)
4. Copy your **Project URL** and **Service Role Key** (anon key is not enough ‚Ä?use service role for server)
...
2. Sign up / Sign in
3. Create a new project ‚Ü?Node.js (server) + React/JavaScript (browser)
4. Copy your **DSN** for the Node.js project
5. You'll get separate DSNs for server and browser ‚Ä?save both
...
**In Railway dashboard ‚Ü?Project Settings ‚Ü?Environment:**
...
## üîç Phase 4: Verify Everything Works

### Test the Live App
1. Open `https://your-domain.com` in browser
2. You should see the homepage
3. Click "Connect Board" ‚Ü?you get a board at `https://your-domain.com/board`
4. Phone: scan QR code or visit `https://your-domain.com/controller?code=XXXXXX`
5. Send a message ‚Ä?should flip on the board
...
## üõ°Ô∏?Optional: Add SSL/TLS (Already Done by Railway)
...
| `SUPABASE_URL` | Supabase dashboard ‚Ü?Settings | Format: `https://xxxxx.supabase.co` |
| `SUPABASE_SERVICE_ROLE_KEY` | Supabase ‚Ü?Settings ‚Ü?API Keys | **Not** the anon key |
| `SENTRY_DSN` | Sentry project ‚Ü?Settings | Server DSN (Node.js) |
| `VITE_POSTHOG_KEY` | PostHog ‚Ü?Project Settings | Project API key |
...
- Check browser DevTools ‚Ü?Network ‚Ü?WS/WSS column
...
1. **Sentry Dashboard** ‚Ä?errors are grouped by type; set up alerts for critical errors
2. **PostHog Insights** ‚Ä?track user engagement, session duration, features used
3. **Railway Logs** ‚Ä?watch for WebSocket disconnections or DB errors
4. **Supabase Dashboard** ‚Ä?check session count, last_active timestamps
...
## üìû Quick Recap: What Was Changed
